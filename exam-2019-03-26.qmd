...
a. Are there outliers in the sample? Justify the answer.
  
a. What is the minimum time for the 20% of patients with highest response time?


a. What is the average response time? Is the mean representative?
...
## Solution
a. $Q_1=12.7344$ min, $Q_3=25.8333$ min, $IQR=13.099$ min, $f_1=-6.9141$ min and $f_2=45.4818$ min. Therefore there are outliers in the sample since the upper limit of the last interval is above the upper fence.
a. $P_{80}=27.619$ min.

a. $\bar x=20.7$ min, $s^2=129.1767$ min$^2$, $s=11.3656$ min and $cv=0.5491$. The mean is moderately representative since the $cv\approx 0.5$.

a. $g_1=0.9393$ and $g_2=0.2523$. Since $g_1$ and $g_2$ are between -2 and 2, we can assume that the sample comes from a normal (bell-shaped) population.

a. The standard score of the first sample is $z(25)=0.3783$ and the standard score of the second one is $z(25)=0.4667$, thus a time of 25 min is relatively greater in the second sample.
:::
:::
...
a. <img src="../img/regnol-fis-4-scatterlot-height-age.svg" title="Scatter plot of Heigh on Age" alt="Scatter plot of Height on Age" style="display: block; margin: auto;" width="600"/>
...
a. Logarithmic regression model: $y=24.2639+50.8848\log(x)$.
